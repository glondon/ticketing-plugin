...
/*
Plugin Name: VES
Plugin URI: http://mywebmastertips.com
Description: Simple plugin for in-house ticket management that can be edited manually to fit your current needs
Author: Greg London
Version: 1.2
Author URI: http://mywebmastertips.com
*/
...
This plugin  adds a simple ticket system to VES. This allows VES Employees to submit tickets to
report customer problems. This ticketing system was meant to be used as an in house ticketing system only. So only Admins and Contributors can add tickets. Subscribers and guests are not allowed access. So this is completely private to use in house. 

Contributors have the ability to add, edit, and delete tickets. Users can view other tickets by coworkers, but are not allowed to edit or delete them. This plugin creates a custom database table that can be modified.

There is a search option on the view page.
...
function ves_get_ticket_prefix() {
	...
	if ($user_email == 'G@v.com') {
		...
		$pre = 'HA';
	} elseif ($user_email == 'M@v.com') {
		$pre = 'MF';
	} elseif ($user_email == 'W@v.com') {
		$pre = 'WM';
	} elseif ($user_email == 'T@v.com') {
		$pre = 'TM';
	} elseif ($user_email == 'S@v.com') {
		$pre = 'SP';
	} elseif ($user_email == 'M@v.com') {
		$pre = 'MW';
	} elseif ($user_email == 'J@v.com') {
		$pre = 'JW';
	} elseif ($user_email == 'F@v.com') {
		$pre = 'FR';
	} else {
		$pre = 'NA';
	}
...
function ves_deactivation()
{
	...
	// simply write the code to drop the table is want the ids to start from scratch....
...
// note this is only for v server mail use....
